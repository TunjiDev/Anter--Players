  ...
  const submitHandler= async(e)=>{
    ...
    return;
      ...
      .then((data) => {
        ...
        dispatch({type:'ADDINFO', payload:{username:data.user.username, profilePicture:data.user.profilePicture}})
    ...
    <div>   
      <div className="chooseUser-container">
        <div className="user-profile-photo-container">
          <div className="user-profile-photo-container__pp">   
            <div className="pp-image-container">     
              ...
              <div className='ion-spinner-container'> {loading &&<IonSpinner name="bubbles" className='ion-spinner' />}</div>
            </div>
          </div>
        </div>
        <div className="userName-container">
          ...
          <div className={`userName-input-container ${error && 'error-boundary'}`}>
            <input
              value={username}
          ...
          {error && <p style={{color:'red', textAlign:'center', fontSize:'1.3rem'}}>Username must not be empty</p>}
          {serverError && <p style={{color:'red', textAlign:'center', fontSize:'1.3rem'}}>{serverError}</p>}
          <p className="userName-text">
            Your username will be visible to your contact
          </p>
          <button onClick={submitHandler} className="btn splash-screen__btn link-btn">
          {isLoading? <IonSpinner name="bubbles"  />: <p>Next</p>}
          </button>
        </div>
      </div>
    </div>
